...
nv.models.multiBarChart = function() {
  ...
  xAxis
    ...
    .tickPadding(0)
    //.highlightZero(false)
    //.axisLabel('Counties') //add axis label to bottom
    .showMaxMin(false) //hides or shows bottom label on x axis
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      if (showControls) {
        var controlsData = [
          { key: 'By SPLOST', disabled: multibar.stacked() },
          { key: 'Total', disabled: !multibar.stacked() }
        ];

        controls.width(controlWidth()).color(['#444', '#444', '#444']);
        g.select('.nv-controlsWrap')
            .datum(controlsData)
            .attr('transform', 'translate(-24,' + (-margin.top) +')')
            .call(controls);
      }
      ...
      if (staggerLabels) {
          ...
          xTicks
            .selectAll("text")
            .style('text-anchor', 'end')
            .attr('transform', 'translate(0,10) rotate(-67.5 0,0)');
